...
from pyoperators.decorators import (
    contiguous, involutary, inplace_only, orthogonal)
...
class AxisPermutationOperator(Operator):
    def __init__(self, axes, **keywords):
        ...
        if all(a == tuple(r) for a, r in zip(axes[ndmin:],
                                             self._identity_axes()[ndmin:])):
            self.__class__ = IdentityOperator
            self.__init__(**keywords)
            return
        Operator.__init__(self, **keywords)
        self.axes = axes
        self.set_rule('I', lambda s: AxisPermutationOperator(
            [np.argsort(a) if a is not None else None for a in s.axes]))
        self.set_rule(('.', AxisPermutationOperator), self._rule_composition,
                      CompositionOperator)
    ...
    def _rule_composition(self, other):
        ndmin = max(ifirst_is_not(self.axes, None),
                    ifirst_is_not(other.axes, None))
        new_axes = ndmin * [None] + \
                   [[self.axes[i][other.axes[i][j]] for j in range(i)]
                    for i in range(ndmin, 32)]
...
class AxisRollOperator(AxisPermutationOperator):
    def __init__(self, axis, start=0, **keywords):
        axis = int(axis)
        start = int(start)
        ndmin = max(axis + 1 if axis >= 0 else -axis,
...
class AxisSwapOperator(AxisPermutationOperator):
    def __init__(self, axis1, axis2, **keywords):
        ...
        axes = self._identity_axes()
        ndmin = max(axis1 + 1 if axis1 >= 0 else -axis1,
                    axis2 + 1 if axis2 >= 0 else -axis2)
        axes[:ndmin] = ndmin * (None, )
        for n in range(ndmin, 32):
...
class ReshapeOperator(Operator):
    ...
    def __init__(self, shapein, shapeout, **keywords):
        ...
        self.set_rule(('.', ReshapeOperator), self._rule_composition,
                      CompositionOperator)
