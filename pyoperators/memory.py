...
class MemoryPool(object):
    ...
    def copy_if(self, v, aligned=False, contiguous=False):
        ...
        if aligned and address % MEMORY_ALIGNMENT != 0 or \
    ...
    def extract(self, shape, dtype, aligned=False, description=None,
                verbose=None):
        """
        ...
        compatible = lambda x: iscompatible(x, nbytes, aligned,
                                            MEMORY_TOLERANCE)
        try:
    ...
    def __str__(self, **keywords):
        ...
        d = dict((v.__array_interface__['data'][0] if isinstance(v, np.ndarray)
                 else v, k) for k, v in keywords.items())
        result = []
