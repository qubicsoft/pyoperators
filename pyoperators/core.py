...
OPERATOR_ATTRIBUTES = [
    'attrin', 'attrout', 'classin', 'classout', 'commin', 'commout',
    'redtypein', 'redtypeout', 'reshapein', 'reshapeout',
    'restridesin', 'restridesout', 'shapein', 'shapeout',
    'toshapein', 'toshapeout', 'dtype', 'flags']
...
class Operator(object):
    ...
    """
    def __init__(
            self, direct=None, transpose=None, adjoint=None, conjugate=None,
            inverse=None, inverse_transpose=None, inverse_adjoint=None,
            inverse_conjugate=None, attrin={}, attrout={}, classin=None,
            classout=None, commin=None, commout=None, redtypein=None,
            redtypeout=None, reshapein=None, reshapeout=None, restridesin=None,
            restridesout=None, shapein=None, shapeout=None, toshapein=None,
            toshapeout=None, validate_dtypein=None, validate_dtypeout=None,
            validate_shapein=None, validate_shapeout=None,
            validate_stridesin=None, validate_stridesout=None,
            dtype=None,
            flags={}, name=None):
        ...
        self._init_inout(
            attrin, attrout, classin, classout, commin, commout, redtypein,
            redtypeout, reshapein, reshapeout, restridesin, restridesout,
            shapein, shapeout, toshapein, toshapeout, validate_dtypein,
            validate_dtypeout, validate_shapein, validate_shapeout,
            validate_stridesin, validate_stridesout)
    ...
    def redtypein(self, dtype):
        ...
        if dtype is not None and dtype.kind not in 'biufc':
            raise TypeError(
                "The input data type '{0}' is not numerical.".format(dtype))
    ...
    def restridesin(self, strides, shape, dtype, inplace):
        ...
        """
        if self.flags.contiguous_input and strides is not None and \
           strides != strides_contiguousarray(shape, dtype):
            raise Mismatch()
        if inplace:
            return strides
    ...
    def _init_inout(
            self, attrin, attrout, classin, classout, commin, commout,
            redtypein, redtypeout, reshapein, reshapeout, restridesin,
            restridesout, shapein, shapeout, toshapein, toshapeout,
            validate_dtypein, validate_dtypeout, validate_shapein,
            validate_shapeout, validate_stridesin, validate_stridesout):
        """
        ...
        if flag_is == 'explicit':
            self.reshapeout = Operator.reshapeout.__get__(self, type(self))
            self.validate_shapein = Operator.validate_shapein.__get__(
                self, type(self))
    ...
    def _init_inout_attribute_square(self, attr):
        ...
        if attrin.im_func is getattr(Operator, attr + 'in').im_func and \
           attrout.im_func is not getattr(Operator, attr + 'out').im_func:
    ...
    def _validate_arguments(self, input, output):
        ...
        if output is not None:
            ...
            if not iscompatible(output, output.shape, dtype,
               self.flags.aligned_output) or isalias(input, output) and \
               not self.flags.inplace:
...
class CommutativeCompositeOperator(CompositeOperator):
    ...
    def _get_attributes(cls, operands, **keywords):
        attr = {
            ...
            'validate_shapein': cls._merge_attribute('validate_shapein',
                                                     operands),
            'validate_shapeout': cls._merge_attribute('validate_shapeout',
                                                      operands),
            'validate_stridesin': cls._merge_attribute('validate_stridesin',
                                                       operands),
            'validate_stridesout': cls._merge_attribute('validate_stridesout',
                                                        operands)}
        ...
        for igroup, (ninplace, bufsize, aligned, contiguous) in renumerate(
            ...
            if igroup != ngroups - 1:
                ...
                reuse_output = not preserve_output and (igroup % 2 == 0) and \
                    iscompatible(output, bufsize, np.int8, aligned) and \
                    not isalias(output, i) or \
                    igroup == 0
                if reuse_output:
                    o_ = output
                else:
    ...
    for attr in OPERATOR_ATTRIBUTES:
        if attr != 'flags':
            ...
            if attr in ('reshapein', 'reshapeout', 'toshapein', 'toshapeout',
                        'validate_shapein', 'validate_shapeout',
                        'validate_stridesin', 'validate_stridesout'):
