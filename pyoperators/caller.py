...
from .utils import (
    groupbykey, ifirst, isalias, operation_assignment, product, tointtuple)
import core
...
class Requirement(object):
    ...
    def __str__(self):
        return '({0}, {1}, {2}, {3})'.format(self.dtype, self.shape,
                                             self.strides, self.aligned)
    __repr__ = __str__
...
class CallingSequence(object):
    ...
    """
    def __init__(self, operators, (dtypein, shapein, stridesin, alignedin),
                 (dtypeout, shapeout, stridesout, alignedout),
                 preserve_input, preserve_output, alias):
    ...
    def analyse_preserve_input(self, preserve_input):
        ...
        self.first_group_is_inplace = not preserve_input and \
                                      self.operators[0].flags.inplace
    ...
    def analyse_requirements(self, alignedin, stridesin, alignedout,
                             stridesout):
        """
    ...
    def find_candidates_alignment(self, bl, alignedin, alignedout):
        ...
        if not alignedout:
            ...
            try:
                index = ifirst(ops, lambda x: x.flags.aligned_output,
                               reverse=True)
    ...
    def find_candidates_strides(self, bl, stridesin, stridesout):
        ...
        if stridesout is not None and strides is not None and \
           strides != stridesout:
            pass
    ...
    def analyse_buffer_usage(self):
        ...
        self.reuse_input = not self.preserve_input and noutplaces > 2 and \
                           isvalid(b.shape, b.dtype, b.strides)
        b = self.requirements[-1]
        self.reuse_output = not self.preserve_output and \
                            isvalid(b.shape, b.dtype, b.strides)
    ...
    def split_group(self, igroup, index):
        ...
        if index < 0 or index > n:
            raise ValueError("Invalid index value '{0}' ({1}<=index<={2}).".
                             format(index, 0, n))
        if index in (0, n):
...
class CallingSequenceManager(object):
    ...
    def __call__(self, input, output, operation, preserve_input):
        ...
        try:
            ...
            if output is None:
                output = call.get_output()
                infoout = self.get_info(output)
                signature = (infoin, infoout, preserve_input, preserve_output,
                             False)
                self.signatures[signature] = call
    ...
    def get_info(x):
        ...
        return (x.dtype, x.shape, x.strides,
