...
from .core import (
    Operator, BlockRowOperator, BroadcastingOperator, CompositionOperator,
    DenseOperator, DiagonalOperator, HomothetyOperator, IdentityOperator,
    ReductionOperator, Mismatch, _pool)
...
class SwapAxesOperator(Operator):
    ...
    def __init__(self, axis1, axis2, **keywords):
        ...
        self.set_rule('I', l)
        self.set_rule('T', l)
        self.set_rule(('.', type(self)), self._rule_swapaxes,
                      CompositionOperator)
    ...
    def reshapein(self, shape):
        shape = list(shape)
        shape[self.axis1], shape[self.axis2] = \
            shape[self.axis2], shape[self.axis1]
        return shape
