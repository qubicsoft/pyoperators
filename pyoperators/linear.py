...
from .core import (
    ...
    DenseBase, DenseOperator, DiagonalOperator, HomothetyOperator,
    ReductionOperator, _pool)
from .memory import empty
from .utils import (
    cast, complex_dtype, float_dtype, izip_broadcast, pi, product, strshape,
    tointtuple)
...
class DenseBlockColumnOperator(DenseBase):
    ...
    """
    def __init__(self, data, broadcast='outer', naxes=None, naxesin=None,
                 naxesout=None, dtype=None, **keywords):
        DenseBase.__init__(self, data, broadcast, naxes=naxes, naxesin=naxesin,
                           naxesout=naxesout, dtype=dtype, **keywords)
        ...
        if self.broadcast == 'inner':
            self.del_rule('T')
        self.set_rule(('.', DenseOperator), self._rule_mul_dense,
                      CompositionOperator)
        self.set_rule((DenseOperator, '.'), self._rule_rmul_dense,
                      CompositionOperator)
    ...
    def _rule_transpose(self):
        ...
        return DenseBlockColumnOperator(
            data, broadcast='outer', naxesin=data.ndim-self.naxesin,
            naxesout=1)
    ...
    def _rule_mul_dense(self, other):
        if self.naxesin != other.naxesout:
            return
        if self.naxesin != 1 or self.naxesout != 1 or \
           other.naxesin != 1 or other.naxesout != 1:
            return  # not implemented
        data = np.dot(self.data, other.data)
        return DenseBlockColumnOperator(
            data, broadcast=self.broadcast, naxesin=other.naxesin,
...
class DenseBlockDiagonalOperator(DenseBase):
    def __init__(self, data, broadcast='outer', naxes=None, naxesin=None,
        ...
        self.set_rule((DenseOperator, '.'), self._rule_rmul_dense,
                      CompositionOperator)
        self.set_rule(('.', DenseBlockColumnOperator), self._rule_mul_bc,
                      CompositionOperator)
        self.set_rule(('.', DenseBlockDiagonalOperator), self._rule_mul_bd,
                      CompositionOperator)
    ...
    def _rule_transpose(self):
        ...
        return DenseBlockDiagonalOperator(
            data, broadcast=self.broadcast, naxesin=self.naxesout,
    ...
    def _rule_mul_bc(self, other):
        ...
        return DenseBlockColumnOperator(data, naxesin=other.naxesin,
                                        naxesout=self.naxesout)
    ...
    def _rule_mul_bd(self, other):
        ...
        # not tested
        data = np.einsum('...ij,...jk->...ik', self.data, other.data)
        return DenseBlockDiagonalOperator(data, naxesin=other.naxesin,
                                          naxesout=self.naxesout)
...
class Rotation2dOperator(DenseBase):
    ...
    """
    def __init__(self, angle, degrees=False, block_diagonal=False, dtype=None,
                 **keywords):
        ...
        keywords['flags'] = self.validate_flags(
            keywords.get('flags', {}),
            orthogonal=m.ndim == 2 or block_diagonal)
        self.__class__ = DenseBlockDiagonalOperator if block_diagonal else \
            DenseBlockColumnOperator
